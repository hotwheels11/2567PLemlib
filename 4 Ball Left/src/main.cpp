...
void autonomous() {
	...
	chassis.swingToHeading(-60, lemlib::DriveSide::LEFT,500,{.earlyExitRange=3});//Swing to first 3 blocks
	chassis.waitUntil(7);
	Matchload.set_value(1); //Deploys grabbing blocks
	chassis.waitUntilDone();
	chassis.moveToPoint(-4,25,500);
	chassis.turnToHeading(-128,500); //Turns to Matchload
	chassis.moveToPoint(-26.7,11,1300,{.maxSpeed=77}); //Drives infront of Matchload
	chassis.turnToHeading(-178,600); //Turns to Matchloader
	//chassis.moveToPoint(-31,30,1000,{.forwards=false,.maxSpeed=40}); //Drives into long goal
    chassis.moveToPoint(-31,30.001,1000,{.forwards=false,.maxSpeed=40});
    chassis.waitUntilDone();
	Hood.set_value(1);
	...
	chassis.moveToPoint(-18.7,11,1200,{.forwards=false,.maxSpeed=77});
	chassis.turnToHeading(180,700);
	Descore.set_value(0);
	chassis.moveToPoint(-18.7,49,2000,{.forwards=false,.maxSpeed=50});
}
