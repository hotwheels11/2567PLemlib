...
void autonomous() {
    Descore.set_value(1);
    chassis.moveToPoint(0,30.5, 940,{.maxSpeed=120}); //Drives from start
    chassis.turnToHeading(86.8,460); //Turns to matchloader
    Matchload.set_value(1);
    chassis.moveToPoint(14,34.5,850,{.maxSpeed=100}); //Goes into matchloader
    intakeFront.move(127);
    chassis.moveToPoint(-28,34.5,1600,{.forwards=false,.maxSpeed=100}); //Backs into long goal
    chassis.waitUntil(26);
    Matchload.set_value(0);
    intake.move(127);
    chassis.waitUntilDone();
    intake.move(0);
    chassis.moveToPoint(4.5, 35, 900,{.maxSpeed=100}); //Pulls out of Long goal'
    intakeFront.move(-127);
    chassis.turnToHeading(215, 550); //3 stack
    intakeFront.move(127);
    chassis.moveToPoint(-28.5, -3.75, 1300,{.maxSpeed=100}); //Goes to lower goal
    ...
    intakeFront.move(-127);
    pros::delay(1000);
    chassis.moveToPoint(-6.7,17.15,900,{.forwards=false,.maxSpeed=100}); //Backs out of lower goal
    chassis.turnToHeading(264.5,460); //Turns parallel to Long goal
    Descore.set_value(0);
    chassis.moveToPoint(-35, 14.2, 1000, {}); //Descores long goal fast
}
